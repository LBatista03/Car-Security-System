...
C_VibrationSensor_Window::C_VibrationSensor_Window() : i2c_fd(-1), vibration_window_Threshold(3.0f) { 
   i2c_fd = init_i2c_fd();
    if (i2c_fd < 0) {
        std::cerr << "Failed to initialize I2C for C_VibrationSensor_Window" << std::endl;
    }
    std::cout << "ADC inicializado com sucesso." << std::endl;
}

C_VibrationSensor_Window::~C_VibrationSensor_Window() {
    if(i2c_fd >= 0) {
        close(i2c_fd);
        i2c_fd = -1;
    }
...
bool C_VibrationSensor_Window:: CheckForTrigger(double value_read){
    trigger=false;
    if(value_read>vibration_window_Threshold){
        trigger=true;
        std::cout << "Window Vibration Sensor Triggered" <<std::endl;
    }
        return trigger;
}

double C_VibrationSensor_Window:: getValue() {
   // value=4.5f;
    int adc_value_a1 = read_channel(ADC_CHANNEL_1);
    if (adc_value_a1 < 0){
        std::cout << "Window Vibration sensor read error"<<std::endl;
    }
    double voltage_a1 = (adc_value_a1 / 32768.0) * 4.096;
    return voltage_a1;
}

int C_VibrationSensor_Window :: init_i2c_fd(){
...
int C_VibrationSensor_Window :: read_channel(uint16_t mux_config) {
